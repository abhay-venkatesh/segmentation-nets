...
class EDSegNet:
    ...
    def deconv_layer(bottom, filter_shape, output_shape, name, strides=[1,2,2,1], padding="SAME"):
        ...
        init_w = tf.constant_initializer(value=weights,dtype=tf.float32)
        filt = tf.get_variable(name="%s_w"%name,initializer=init_w,shape=weights.shape,dtype=tf.float32)
        deconv = tf.nn.conv2d_transpose(bottom, filt, output_shape, strides=strides, padding='SAME', name = name)
        init_b = tf.constant_initializer(value=0.0, dtype=tf.float32)
        bias = tf.get_variable(name="%s_b"%name,initializer=init_b,shape=[filter_shape[2]],dtype=tf.float32)
