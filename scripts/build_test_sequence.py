"""
Script that constructs a small test sequence of 20 image-gt pairs.
...
def convert_image(dirName, fname, counter):
  ...
  outfile = '../datasets/Sequence-20-Unseen/ground_truths/seg' + str(counter) + '.png' 
  cv2.imwrite(outfile,res*8)
  image_outfile = '../datasets/Sequence-20-Unseen/images/pic' + str(counter) + '.png'
  copyfile(filePath.replace('seg','pic'), image_outfile) 

def build():

  # Build output directories
  output_directory = '../datasets/Sequence-20-Unseen/' 
  if not os.path.exists(output_directory):
    os.makedirs(output_directory) 
  ground_truths_directory = '../datasets/Sequence-20-Unseen/ground_truths/'
  if not os.path.exists(ground_truths_directory):
    os.makedirs(ground_truths_directory) 
  scene_image_directory = '../datasets/Sequence-20-Unseen/images/'
  if not os.path.exists(scene_image_directory):
    os.makedirs(scene_image_directory) 

  source_directory = "../../../../UnrealEngineSource/batch9/round19/"
